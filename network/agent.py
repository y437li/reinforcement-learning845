...
class PedraAgent():
    ...
    def reward_gen(self, d_new, action, crash_threshold, thresh, debug, x_pos,y_pos,f_x_pos,f_y_pos,star_time,cfg):
        ...
        #if c_new < crash_threshold or L_new < crash_threshold or R_new < crash_threshold:
            done = True
            arrive = False
            reward = -10
        #elif x_pos == f_x_pos and y_pos == f_y_pos:
            #arrive = True
            #reward = 100
        else:
            arrive = False
    ...
    def reward_gen2(self,star_time):
        done = False
        elapsed_time = time.time() - star_time
        reward = elapsed_time*-1
        return reward, done
